...
func Plugin(ctx context.Context) *plugin.Plugin {
	p := &plugin.Plugin{
		...
		TableMap: map[string]*plugin.Table{
			...
			"mongodbatlas_database_user":                     tableAtlasDatabaseUser(ctx),
			"mongodbatlas_custom_db_role":                    tableAtlasCustomDBRole(ctx),
			"mongodbatlas_x509_authentication_database_user": tableAtlasX509Auth(ctx),
			"mongodbatlas_project_event":                     tableAtlasProjectEvents(ctx),
			"mongodbatlas_org_event":                         tableAtlasOrgEvents(ctx),
			"mongodbatlas_team":                              tableAtlasTeam(ctx),
			"mongodbatlas_container":                         tableAtlasContainer(ctx),
		},
	}
